...
\subsection{Structure and Repitition}
...
Each of these relationships also has its inverse, e.g. NotIsA. We can use these relationships, along with a few of our own listed below, to build a cohesive story about particular characters during the generation phase.
...
These complexities are further compounded by the fact that we cannot rely on correct grammar in poems. We therefore need a \textit{semantic} parse. Noah's ARK is an informal research group run by Noah Smith at Carnegie Mellon University [REFERENCE]. They provide online API access to two tools for linguistic structure analysis, \textit{SEMAFOR} and \textit{TurboParser}. Both of these tools can be used in conjunction to extract ConceptNet relations from natural language.
...
{\centering\textit{"The shopkeeper told the hoodlum to go away"}}
We wish to recognise the verb \textit{'to tell'} as the coordinating word (called the \textbf{target}}), \textit{'the shopkeeper'} as the speaker, \textit{'go away'} as the message and \textit{'the hoodlum'} as the addressee. This can be seen clearly in Figure BLAH. 
...
These labels are retrieved through the SEMAFOR tool, which was trained on FrameNet data to determine the frame-semantic structure of the text. FrameNet [REFERENCE] is a lexical database of \textit{semantic frames}, which describe the meaning of a word based on the words that typically participate with it, known as \textit{frame elements}. It is based on the Frame Semantics Theory, introduced by Charles J.Fillmore et al.[REFERENCE]
